    ...
    public class SnowmanHeuristic implements SearchHeuristic {
        ...
        public float computeEstimate(State state) {
            try {
                ...
                // --- 5. Epsilon Tie-Breaking: IL GRADIENTE PERFETTO ---
                double tie_breaker = 0.0;

                // 1. Penalità palle a metà (Soft Anti-Interleaving)
                int unfinishedSize2 = 0;
                for (int c : size2Count)
                    unfinishedSize2 += c;
                tie_breaker += unfinishedSize2 * 0.5;
                ...
                if (charLocId >= 0 && !remainingBallsList.isEmpty()) {
                    ...
                    long minGroupHash = Long.MAX_VALUE;

                    // 2. STABLE FOCUS GROUP SELECTION (Usa l'Hash del Configurator)
                    for (int i = 0; i < remainingTargets; i++) {
                        ...
                        if (loc0 >= 0 && loc1 >= 0 && loc2 >= 0) {
                            if (distanceMatrix[loc0][loc1] < models.SnowmanConfigurator.UNREACHABLE)
                                groupDist += distanceMatrix[loc0][loc1];
                            if (distanceMatrix[loc1][loc2] < models.SnowmanConfigurator.UNREACHABLE)
                                groupDist += distanceMatrix[loc1][loc2];
                            if (distanceMatrix[loc0][loc2] < models.SnowmanConfigurator.UNREACHABLE)
                                groupDist += distanceMatrix[loc0][loc2];
                        }

                        long gHash = g.getBalls().get(0).getIntId() * 100L + g.getBalls().get(1).getIntId() * 10L
                                + g.getBalls().get(2).getIntId();

                        boolean isBetter = false;
                        if (gSizeSum > maxGroupSize)
                            isBetter = true;
                        else if (gSizeSum == maxGroupSize) {
                            if (groupDist < minGroupDistance - 0.0001)
                                isBetter = true;
                            else if (Math.abs(groupDist - minGroupDistance) <= 0.0001 && gHash < minGroupHash)
                                isBetter = true;
                        }
                    ...
                    if (focusGroup != null) {
                        // 3. HARD CANTIERE CHIUSO (+2.0)
                        if (maxGroupSize > 3) {
                            for (Ball rb : remainingBallsList) {
                                if (rb.getSize() > 1) {
                                    boolean inFocus = false;
                                    for (Ball fb : focusGroup.getBalls()) {
                                        if (fb.getIntId() == rb.getIntId()) {
                                            inFocus = true;
                                            break;
                                        }
                                    }
                                    // Penalità severa ma scalabile. Non causerà enormi esplorazioni a vuoto.
                                    if (!inFocus)
                                        tie_breaker += 2.0;
                                }
                            }
                        ...
                        // 4. GRAVITÀ DEL FOCUS (Mantenere le palle vicine)
                        tie_breaker += minGroupDistance * 0.01;

                        // 5. REPULSIONE E CONGESTIONE (Calibrate)
                        ...
                        for (int i = 0; i < remainingBallsList.size(); i++) {
                            ...
                            for (Ball fb : focusGroup.getBalls()) {
                                if (fb.getIntId() == b1.getIntId()) {
                                    b1Focus = true;
                                    break;
                                }
                            }

                            // Repulsione: +0.001. Essendo minore del guadagno di movimento (+0.002),
                            // l'omino attraverserà i corridoi affollati senza esitare!
                            ...
                            for (int j = i + 1; j < remainingBallsList.size(); j++) {
                                ...
                                for (Ball fb : focusGroup.getBalls()) {
                                    if (fb.getIntId() == b2.getIntId()) {
                                        b2Focus = true;
                                        break;
                                    }
                                }
                                // Congestione: +0.1 se due palle inutili si incastrano a vicenda
                        ...
                        if (minDistToFocus < models.SnowmanConfigurator.UNREACHABLE) {
                            // Motore principale: +0.002 per ogni passo di lontananza.
                            tie_breaker += minDistToFocus * 0.002;
                        }
                    } else {
